...
所有已登录用户都可以在“Tool 清单”页签查看当前 Markdown 文档。只有用户名大小写严格为 `Chent` 的账号可以上传或替换 `.md` 文件；文件最大 1 MiB，上传权限由服务端强制校验，内容保存在 SQLite 数据库并记录审计日志。上传的新文档会替换当前清单，页面使用 GitHub Flavored Markdown 渲染表格、列表和代码块，原始 HTML 不会执行。
...
投产管理页面在日期分组下继续按 Skill 分组展示，同一日期内不同 Skill 不再混在一个列表中；桌面端以列布局并排显示各 Skill，下面按版本号逐行对齐，每个 Skill 分组仍会列出该日期下的全部已批准版本，并保留单版本调整、改期申请和审批操作。
...
从旧版本升级时，建议先按上述方式备份数据库，再重新构建镜像。应用启动后会自动新增所需的 `release_time` 字段、密码重置申请表、投产日期变更申请表和 Tool 清单表；已有的已批准版本会使用原审批日期作为初始投产日期，已有待审版本可由审核者在批准时补填星期五日期。账号、密码哈希、版本和审计数据不会被清除。
