...
export default function ReleasesPage() {
  ...
  return <div className="page release-page">
    ...
    {data?.groups.length ? <>
      ...
      <div className="release-groups">{groups.map((group) => {
        const maxVersions = Math.max(...group.skills.map((skill) => skill.versions.length), 0);
        ...
        <div className="release-skill-columns" style={{ '--release-skill-count': group.skills.length }}>
          ...
          {Array.from({ length: maxVersions }, (_, index) => group.skills.map((skill) => {
            ...
            return <div className="release-skill-cell" key={version.id}><div className={`release-version-row ${user.role === 'ADMIN' ? 'admin-row' : 'request-row'}`}>
