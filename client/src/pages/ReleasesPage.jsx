  ...
  return <div className="page release-page">
    ...
    {data?.groups.length ? <>
      ...
      <div className="release-groups">{groups.map((group) => <section className="panel release-group" key={group.releaseTime}>
        ...
        <div className="release-skill-list">{group.skills.map((skill) => <section className="release-skill-group" key={skill.skillId}>
          <div className="release-skill-heading"><Link className="skill-glyph" to={`/skills/${skill.slug}`}>{skill.slug.slice(0, 1).toUpperCase()}</Link><Link to={`/skills/${skill.slug}`}><strong>{skill.slug}</strong></Link><span>{skill.versions.length} 个版本</span></div>
          <div className="release-version-list">{skill.versions.map((version) => {
