...
export default function ReviewPage() {
  ...
  useEffect(() => { setReleaseTime(versionData?.version?.releaseTime || ''); }, [versionData?.version?.releaseTime]);
  ...
  async function review(decision) {
    ...
    if (decision === 'APPROVE' && !releaseTime) return setActionError('批准前必须确认投产日期');
    if (decision === 'APPROVE' && !isFridayReleaseDate(releaseTime)) return setActionError('最终投产日期只能选择星期五');
    const prompt = decision === 'APPROVE' ? `批准后将成为当前发布版本，并归入 ${formatReleaseDate(releaseTime)} 投产批次，确认继续？` : '确认驳回此版本？';
    if (!window.confirm(prompt)) return;
    setBusy(true); setActionError('');
    try {
      await api(`/versions/${id}/review`, { method: 'POST', body: { decision, comment, releaseTime } });
      await reload();
  ...
  return <div className="page review-page"><div className="crumbs"><Link to={user.role === 'REVIEWER' ? '/reviews' : `/skills/${version.slug}`}>← 返回</Link></div><PageHeader eyebrow="VERSION REVIEW" title={`${version.slug} · v${version.versionNo || '草稿'}`} description={version.summary || '暂无变更说明'} actions={<StatusBadge status={version.status} />} /><div className="review-meta"><div><span>提交人</span><strong>{version.creatorName}</strong></div><div><span>提交时间</span><strong>{formatTime(version.submittedAt)}</strong></div><div><span>投产日期</span><strong>{formatReleaseDate(version.releaseTime)}</strong></div><div><span>变更类型</span><strong>{version.changeType === 'ROLLBACK' ? '历史回滚' : version.changeType === 'CREATE' ? '新建 Skill' : '内容更新'}</strong></div><div><span>文件变化</span><strong>{diffData.diff.length} 个文件</strong></div></div>{version.status === 'SUPERSEDED' && <Notice type="warning">该提交基于旧发布版本，已不能批准。编辑者需要从当前版本重新创建草稿。</Notice>}{version.reviewComment && <Notice type={version.status === 'REJECTED' ? 'danger' : 'info'}>审核意见：{version.reviewComment}</Notice>}{actionError && <div className="form-error">{actionError}</div>}<section className="panel diff-panel"><div className="panel-heading"><div><h2>逐文件差异</h2><p>红色为删除，绿色为新增</p></div></div>{diffData.diff.length ? diffData.diff.map((file) => <FileDiff file={file} key={file.path} />) : <div className="empty-diff">该版本与基线内容完全一致</div>}</section>{user.role === 'REVIEWER' && version.status === 'SUBMITTED' && <section className="review-bar"><div className="review-fields"><label className="review-release-field"><span>最终投产日期（仅限周五）</span><input type="date" min="1970-01-02" step="7" value={releaseTime} onChange={(e) => { const value = e.target.value; setReleaseTime(value); if (value && !isFridayReleaseDate(value)) setActionError('最终投产日期只能选择星期五'); else setActionError(''); }} required /></label><label><span>审核意见</span><textarea value={comment} onChange={(e) => setComment(e.target.value)} maxLength={1000} placeholder="批准时选填；驳回时必须说明需要修改的内容…" /></label></div><div className="review-actions"><button className="button reject" disabled={busy} onClick={() => review('REJECT')}>驳回修改</button><button className="button approve" disabled={busy || version.createdBy === user.id} title={version.createdBy === user.id ? '不能审核自己的提交' : ''} onClick={() => review('APPROVE')}>批准并发布</button></div></section>}</div>;
}
